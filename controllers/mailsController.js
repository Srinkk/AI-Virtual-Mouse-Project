...
const createNewMail = asynchandler(async(req, res) => {
    ...
    if (body.length > 300 ) {
        return res.status(400).json({ message: 
            'Application body is too long.It must be of within 300 characters.' })
    }
    ...
    const mailObj = { subject, days, body, senderId, senderName: 
        sender[0].name, receiverId, receiverName: receiver[0].name}

    const newMail = await Mail.create(mailObj)

    if (newMail) {
        res.status(200).json({ message: `New mail ${newMail.subject}
         sent to ${newMail.receiverName} from ${newMail.senderName}.` })
    } else {
...
const deleteMail = asynchandler(async(req, res) => {
    ...
    `Mail ${result.subject} with Id ${result._id} sent to ${result.receiver}
     by ${result.sender} deleted.` })
    } 
})
...
const updateMailStatus = asynchandler(async(req, res) => {
    ...
    if (!updatedMail) {
        ...
        res.status(200).json({ message: 
        `Status of mail with id ${updatedMail._id} was changed to ${status}.`})
    }
})
