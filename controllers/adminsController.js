...
const getAdmin = asynchandler(async(req, res) => {
    ...
    res.status(200).json({ id: admin[0].id, name: admin[0].name, inst_id: school._id,
         inst_name: admin[0].inst_name, contact: school.contact, code: school.code})
}) 
...
const createNewAdmin = asynchandler(async(req, res) => {
    ...
    if (duplicate) {
        return res.status(409).json({ message: `An admin account already exists with this
         Id in ${inst_name}` })
    }
